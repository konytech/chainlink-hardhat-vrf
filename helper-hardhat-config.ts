...
const getNetworkIdFromName = async (networkIdName:string) => {
    for (const id in networkConfig) {
        if (networkConfig[id as keyof typeof networkConfig]['name'] == networkIdName) {
            return id
        }
    }
    return null
...
const autoFundCheck = async (contractAddr:any, networkName:any, linkTokenAddress:any, additionalMessage:any) => {
    const chainId = await getChainId()
    console.log("Checking to see if contract can be auto-funded with LINK:")
    const amount = +networkConfig[chainId as keyof typeof networkConfig]['fundAmount']
